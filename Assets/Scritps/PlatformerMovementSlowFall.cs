...
public class PlatformerMovementSlowFall : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (velocity.y < 0 && Input.GetButton("Jump") && !Grounded)
        {
            anim1.SetBool("IsSlowFalling", true);
            rb.gravityScale = SlowFallGrav;
            if(velocity.y <= SlowFallSpeed * -1)
            {
                velocity.y = SlowFallSpeed * -1;
            }
        }else
        {
            anim1.SetBool("IsSlowFalling", false);
